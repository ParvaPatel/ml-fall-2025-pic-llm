...
\begin{itemize}[leftmargin=*]
  ...
  \item \textbf{Artefacts.} Log metrics to a new JSON file (e.g., \texttt{artefacts/overfitting\_sweep.json}) so the notebook can add a dedicated plot once runs are complete.
  \item \textbf{Initial Run.} Executed a 30-epoch sweep (batch size 4) on a 43-sequence subset; metrics recorded in \texttt{artefacts/overfitting\_sweep.json} now feed the notebook plot highlighting the divergence between training and testing losses.
\end{itemize}
...
\begin{itemize}[leftmargin=*]
  ...
  \item \textbf{Experimental Grid.}
    ...
    \end{itemize}
  \item \textbf{Metrics.} Log training/testing loss, tokens-per-second, and representative generations. Store results in \texttt{artefacts/hparam\_sweep.json} for notebook visualisation (e.g., heatmaps or spider plots).
  ...
  \item \textbf{Initial Runs.} Logged three configurations (\texttt{emb256/block32/lr5e-4}, \texttt{emb512/block64/lr1e-3}, \texttt{emb1024/block128/lr2e-3}) to \texttt{artefacts/hparam\_sweep.json}; the notebook now visualises their training/testing losses side by side.
\end{itemize}
